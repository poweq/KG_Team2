...
# 바퀴 조인트 이름 설정
LEFT_WHEEL_JOINT = 'wheel1_joint'
RIGHT_WHEEL_JOINT = 'wheel2_joint'
# 키보드 입력과 조인트 변경 매핑
move_bindings = {
    'w': (1.0, 1.0),  # 앞으로 이동
    's': (-1.0, -1.0),  # 뒤로 이동
...
class TeleopWheel(Node):
    def __init__(self):
        ...
        # 현재 조인트 위치 초기화
        self.current_position = {LEFT_WHEEL_JOINT: 0.0, RIGHT_WHEEL_JOINT: 0.0}
    ...
    def publish_joint_state(self):
        # position 값 업데이트 (velocity 값을 누적하여 position 업데이트)
        self.current_position[LEFT_WHEEL_JOINT] += self.current_velocity[LEFT_WHEEL_JOINT] * 0.1  # 0.1은 타이머 주기
        self.current_position[RIGHT_WHEEL_JOINT] += self.current_velocity[RIGHT_WHEEL_JOINT] * 0.1

        # 조인트 상태 메시지 생성
        joint_state = JointState()
        joint_state.header.stamp = self.get_clock().now().to_msg()
        joint_state.name = [LEFT_WHEEL_JOINT, RIGHT_WHEEL_JOINT]
        joint_state.position = [self.current_position[LEFT_WHEEL_JOINT], self.current_position[RIGHT_WHEEL_JOINT]]
